...
class Post {
    ...
    public function submitPost($body, $user_to) {
        $sumit_body = strip_tags($body); // remove html tags
        $sumit_body = mysqli_real_escape_string($this->con, $sumit_body);
        $check_empty = trim($sumit_body); // Deletes all spaces
        ...
        if (mysqli_num_rows($data_query) > 0) {
            ...
            while ($row = mysqli_fetch_array($data_query)) {
                ...
                if ($user_logged_obj->isFriend($added_by)) {
                    ...
                    // Once 5 posts have been loaded, break
                    if ($count > $limit) {
                        break;
                    } else {
                        $count ++;
